...
const (
	...
	SYS_RECVMSG = 27;  // ssize_t recvmsg(int s,_Inout_ _Contains_ptr_ struct msghdr_native *msg,int flags);
	SYS_SENDMSG = 28;  // ssize_t sendmsg(int s,_In_ _Contains_ptr_ const struct msghdr_native *msg,int flags);
	...
	SYS_SIGALTSTACK = 53;  // int sigaltstack(_In_opt_ _Contains_long_ptr_ const struct sigaltstack_native *ss,_Out_opt_ _Contains_long_ptr_ struct sigaltstack_native *oss);
	...
	SYS_READV = 120;  // int readv(int fd,_Inout_updates_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,u_int iovcnt);
	SYS_WRITEV = 121;  // int writev(int fd,_In_reads_opt_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,u_int iovcnt);
	...
	SYS_KTIMER_CREATE = 235;  // int ktimer_create(clockid_t clock_id,_In_ _Contains_long_ptr_ struct sigevent_native *evp,_Out_ int *timerid);
	...
	SYS_AIO_READ = 255;  // int aio_read(_Inout_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	SYS_AIO_WRITE = 256;  // int aio_write(_Inout_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	SYS_LIO_LISTIO = 257;  // int lio_listio(int mode,_Inout_updates_(nent) _Contains_long_ptr_ struct aiocb_native * const *acb_list,int nent,_In_opt_ _Contains_long_ptr_ struct sigevent_native *sig);
	...
	SYS_PREADV = 289;  // ssize_t preadv(int fd,_In_reads_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,u_int iovcnt,off_t offset);
	SYS_PWRITEV = 290;  // ssize_t pwritev(int fd,_In_reads_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,u_int iovcnt,off_t offset);
	...
	SYS_AIO_RETURN = 314;  // ssize_t aio_return(_Inout_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	SYS_AIO_SUSPEND = 315;  // int aio_suspend(_Inout_updates_(nent) _Contains_long_ptr_ struct aiocb_native * const *aiocbp,int nent,_In_opt_ _Contains_long_timet_ const struct timespec *timeout);
	SYS_AIO_CANCEL = 316;  // int aio_cancel(int fd,_In_opt_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	SYS_AIO_ERROR = 317;  // int aio_error(_In_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	...
	SYS_SIGTIMEDWAIT = 345;  // int sigtimedwait(_In_ const sigset_t *set,_Out_opt_ _Contains_long_ptr_ struct siginfo_native *info,_In_opt_ _Contains_long_timet_ const struct timespec *timeout);
	SYS_SIGWAITINFO = 346;  // int sigwaitinfo(_In_ const sigset_t *set,_Out_opt_ _Contains_long_ptr_ struct siginfo_native *info);
	...
	SYS_AIO_WAITCOMPLETE = 359;  // ssize_t aio_waitcomplete(_Outptr_result_maybenull_ _Contains_long_ptr_ struct aiocb_native **aiocbp,_In_opt_ _Contains_long_timet_ struct timespec *timeout);
	...
	SYS_NMOUNT = 378;  // int nmount(_In_reads_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,unsigned int iovcnt,int flags);
	SYS___MAC_GET_PROC = 384;  // int __mac_get_proc(_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_SET_PROC = 385;  // int __mac_set_proc(_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_GET_FD = 386;  // int __mac_get_fd(int fd,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_GET_FILE = 387;  // int __mac_get_file(_In_z_ const char *path_p,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_SET_FD = 388;  // int __mac_set_fd(int fd,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_SET_FILE = 389;  // int __mac_set_file(_In_z_ const char *path_p,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	...
	SYS_SENDFILE = 393;  // int sendfile(int fd,int s,off_t offset,size_t nbytes,_In_opt_ _Contains_ptr_ struct sf_hdtr_native *hdtr,_Out_opt_ off_t *sbytes,int flags);
	SYS_MAC_SYSCALL = 394;  // int mac_syscall(_In_z_ const char *policy,int call,_In_opt_ void *arg);
	SYS___MAC_GET_PID = 409;  // int __mac_get_pid(pid_t pid,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_GET_LINK = 410;  // int __mac_get_link(_In_z_ const char *path_p,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS___MAC_SET_LINK = 411;  // int __mac_set_link(_In_z_ const char *path_p,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	...
	SYS___MAC_EXECVE = 415;  // int __mac_execve(_In_z_ const char *fname,_In_ char **argv,_In_ char **envv,_In_ _Contains_long_ptr_ struct mac_native *mac_p);
	SYS_SIGACTION = 416;  // int sigaction(int sig,_In_opt_ _Contains_ptr_ const struct sigaction_native *act,_Out_opt_ _Contains_ptr_ struct sigaction_native *oact);
	...
	SYS_AIO_FSYNC = 465;  // int aio_fsync(int op,_In_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	...
	SYS_SHM_OPEN = 482;  // int shm_open(_In_z_ const char *path,int flags,mode_t mode);
	...
	SYS_JAIL_GET = 506;  // int jail_get(_In_reads_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,unsigned int iovcnt,int flags);
	SYS_JAIL_SET = 507;  // int jail_set(_In_reads_(iovcnt) _Contains_long_ptr_ struct iovec_native *iovp,unsigned int iovcnt,int flags);
	...
	SYS_WAIT6 = 532;  // int wait6(idtype_t idtype,id_t id,_Out_opt_ int *status,int options,_Out_opt_ _Contains_long_ struct __wrusage *wrusage,_Out_opt_ _Contains_long_ptr_ struct siginfo_native *info);
	...
	SYS_AIO_MLOCK = 543;  // int aio_mlock(_In_ _Contains_long_ptr_ struct aiocb_native *aiocbp);
	...
	SYS_KEVENT = 560;  // int kevent(int fd,_In_reads_opt_(nchanges) _Contains_ptr_ const struct kevent_native *changelist,int nchanges,_Out_writes_opt_(nevents) _Contains_ptr_ struct kevent_native *eventlist,int nevents,_In_opt_ _Contains_long_timet_ const struct timespec *timeout);
